...
channel_statistics=get_channel_stats(youtube, channel_ids) # get channel data
...
for i in range(len(channel_data)):
    ...
    #print(same_id[0])
    if same_id:
        old_st= Yt_st(subscribers=Subscribers, views=Views, videos=Videos, date=Date, youtube_channel_id=same_id[0]) # add youtube_st table if exit in db
        session.add(old_st)
        session.commit() 
        session.rollback()
       
    else:
        new_ct= Yt(channel_id=Channel_id, channel_name=Channel_name, playlist_id=Playlist_id) # add youtube_channel table if not exit in db
        session.add(new_ct)
        session.commit() 
        session.rollback()
        same_id1 =session.query(Yt.id).filter(Yt.channel_id == Channel_id).first() # check id exit in db
        if same_id1:
            old_ct= Yt_st(subscribers=Subscribers, views=Views, videos=Videos, date=Date, youtube_channel_id=same_id1[0])
            session.add(old_ct)
            session.commit() 
            session.rollback()
